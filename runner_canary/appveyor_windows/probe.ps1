...
if ($env:APPVEYOR -notin @('True', 'true')) {
    throw 'This probe is only valid inside AppVeyor.'
}
$appveyorUrl = ([string]$env:APPVEYOR_URL).TrimEnd('/')
if ($appveyorUrl -ne 'https://ci.appveyor.com') {
    throw 'This probe requires the hosted AppVeyor service.'
}
...
$probeTitle = 'WS03-APPVEYOR-UI-PROBE-' + [Guid]::NewGuid().ToString('N')
$marker = 'WS03_INPUT_' + [Guid]::NewGuid().ToString('N')
$uiScriptPath = Join-Path $env:TEMP 'ws03-ui-probe.ps1'
$uiScriptLines = @(
    'Add-Type -AssemblyName System.Windows.Forms',
    'Add-Type -AssemblyName System.Drawing',
...
$hostedEndpoint = $appveyorUrl -eq 'https://ci.appveyor.com'
$workerImage = [string]$env:APPVEYOR_BUILD_WORKER_IMAGE
$freshVmContract = 'appveyor_clean_dedicated_worker_vm'
$physicalDesktopAccepted = ($hostedEndpoint -and [Environment]::UserInteractive -and $sessionId -gt 0 -and $inputDesktopOpened -and $uiaWindowFound -and $uiaTextBoxFound -and $screenshotCaptured -and $screenshotNonUniform -and $sendInputSent -eq $sendInputExpected -and $sendInputEffective)

$result = [ordered]@{
    schema_version = 1
    provider_candidate = 'appveyor_hosted_oss_windows'
    observed_at_utc = [DateTime]::UtcNow.ToString('o')
    appveyor = $true
    appveyor_url = $appveyorUrl
    appveyor_account_name = $env:APPVEYOR_ACCOUNT_NAME
    appveyor_project_slug = $env:APPVEYOR_PROJECT_SLUG
    ...
    build_worker_image = $workerImage
    runner_os = 'Windows'
    fresh_vm_contract = $freshVmContract
